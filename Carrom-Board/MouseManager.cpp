		...
		for (i = 0; i < bricks.getRows(); i++)
		{
			for (j = 0; j < bricks.getCols(); j++)
			{
				if (mx >= board.getX() + bricks.getCell(i, j).getX() && mx <= board.getX() + bricks.getCell(i, j).getX() + bricks.getWidth())
				{
					...
					{
						if (!bricks.getCell(i, j).getStrength() && MouseManager::getButton() == GLUT_LEFT_BUTTON)
						{
							bricks.getCell(i, j).setStrength(1);
							bricks.setRemaining(bricks.getRemaining()+1);
						}
						else if (MouseManager::getButton() == GLUT_RIGHT_BUTTON && bricks.getCell(i, j).getStrength())
						{
...
void MouseManager::click(int button, int state, int mx, int my)
{
	if (state == GLUT_DOWN && (LevelManager::getLevel() == 0 && (menu.getMenu() == MenuManager::GAME_OVER || menu.getMenu() == MenuManager::WINNER)))
	{
		menu.setMenu(MenuManager::MAIN);
		GameManager::reset();
	}
...
void MouseManager::moveHandler(int mx, int my)
{
...
void MouseManager::clickHandler(int button, int state, int mx, int my)
{
	x = mx;
	y = WindowManager::getHeight() - my;
