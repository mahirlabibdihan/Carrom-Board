...
void KeyboardManager::keyHandler(unsigned char key, GLint x, GLint y)	  // Callback When normal key is pressed down
{
	keyboard(key);
	glutPostRedisplay();
}
...
void KeyboardManager::keyboard(unsigned char key)
{
	//if (key == 'q') exit(1);
